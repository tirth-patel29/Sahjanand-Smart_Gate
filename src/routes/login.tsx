...
const search = z.object({ role: z.enum(["resident", "guard", "admin"]).optional() });

export const Route = createFileRoute("/login")({
  validateSearch: (s) => search.parse(s),
  head: () => ({ meta: [{ title: "Sign in — Sahjanand Smart Gate" }] }),
  component: LoginPage,
});
...
function LoginPage() {
  ...
  useEffect(() => {
    if (!loading && session && role) {
      nav({ to: role === "admin" ? "/admin" : role === "guard" ? "/guard" : "/resident" });
    }
